  ...
  wmi.query('SELECT * FROM WmiMonitorID', function (err, result) {
    ...
    names = {}
      for (let monitor of result) {
        ...
        let uid = hwid[2]
        names[uid] = parseWMIString(monitor.UserFriendlyName)
      }
    }
  })
}
...
function getUIDFromWinDDC(monitor) {
...
function getName(monitor) {
  let key = getUIDFromWinDDC(monitor)
  if (key.length > 0 && names.count(key) > 0) {
     return names[key];
  }
